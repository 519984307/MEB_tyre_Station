...
//        url = "/upload/tightendata?";
    ...
    system("eth1 192.168.2.187 netmask 255.255.255.0");
    ...
    if(WIFI_STATE)//服务器
    {
        ...
        else       //Insert
        {
            ...
            if(db2.isOpen() && QSqlDatabase::contains("ReinsertMySQL"))
            {
                ...
                if(!query2.exec("SELECT RecordID, IDCode, ScrewID, Torque, Angle, Curve, TighteningStatus, TighteningTime, Cycle, Operator, Program, JobStatus, Order_id, `Max_Value`, `MinValue`, Channel , Angle_Max , Angle_Min , Torque_Max, Torque_Min FROM "+Localtable +" Where UploadMark =4 and Curve !='null' order by RecordID limit 1"))
                {

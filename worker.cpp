...
void Worker::DataReceived()
{
    ...
    emit havedconnect(true);//拧紧机连接状态发到4700（发数据到拧紧机）线程
    socketFlag4710 = true;
}
/***************************************************/
...
//      qDebug() << "receiveResult" << temp;
    ...
    if(node.nodeName() == "PRC_SST")
    {
        if(!GCalibrationON  && SYSS != "Ready"){
            ...
            {
                QDomElement element1 = node.toElement().childNodes().at(i).toElement();
                if(element1.nodeName()=="PAR")
                {
                    int PAR_child = element1.childNodes().length();
                    for(j=0;j<PAR_child;j++)
                    {
                         QDomElement element2 = element1.childNodes().at(j).toElement();
                        if(element2.nodeName()=="FAS")
                        {
                            int FAS_child = element2.childNodes().length();
                            for(k=0;k<FAS_child;k++)
                            {
                                QDomElement element3 = element2.childNodes().at(k).toElement();
                                if(element3.nodeName()=="GRP")
                                {
                                    int GPR_child = element3.childNodes().length();
                                    for(l=0;l<GPR_child;l++)
                                    {
                                        QDomElement element4 = element3.childNodes().at(l).toElement();

                                        if(element4.nodeName() == "GNR"){//新增取GNR的值
                                            QString GNR_VAL = element4.text();
                                            if(GNR_VAL == "44"){
                                                emit sendToPLC(GNR_VAL);
                                            }
                                        }

                                        if(element4.nodeName()=="TIP")//刷到一组TIP进一次，这里有5组
                                        {
                                            int TIP_child = element4.childNodes().length();
                                            for(m=0;m<TIP_child;m++)
                                            {
                                                QDomElement element5 = element4.childNodes().at(m).toElement();
                                                //通道号
                                                if(element5.nodeName()=="KNR")
                                                    Channel = element5.text();
                                                //程序号
                                                if(element5.nodeName()=="PRG")
                                                    Program = element5.text();
                                                //循环号
                                                if(element5.nodeName()=="CSR")
                                                    Cycle_CSR = element5.text();
                                                //日期
                                                if(element5.nodeName()=="DAT")
                                                    DATE_blc = element5.text();
                                                //时间
                                                if(element5.nodeName()=="TIM")
                                                    TIME_blc = element5.text();
                                                //拧紧合格状态
                                                if(Program == "99")
                                                {
                                                    STATE_blc = "NOK";
                                                ...
                                                {
                                                    if(element5.nodeName()=="STA")
                                                    {
                                                        STATE_blc = element5.text();
                                                        GPareXMLFlag = true;
                                                        if(STATE_blc == "IO")
                                                        {
                                                            STATE_blc = "OK";
                                            ...
                                            if(blc!=-1)//数据节
                                            {
                                                QDomElement element_BLC = element4.childNodes().at(blc).toElement();
                                                qDebug()<<"this is blc not equ -1";
                                                int BLC_child = element_BLC.childNodes().length();
                                                for(n=0;n<BLC_child;n++)
                                                {
                                                    QDomElement element_PRO = element_BLC.childNodes().at(n).toElement();
                                                    if(element_PRO.nodeName()=="PRO")
                                                    {
                                                        int PRO_child = element_PRO.childNodes().length();
                                                        for(p=0;p<PRO_child;p++)
                                                        {
                                                            QDomElement element_MAR = element_PRO.childNodes().at(p).toElement();
                                                            if(element_MAR.nodeName()=="MAR")
                                                            {
                                                                int MAR_child = element_MAR.childNodes().length();
                                                                for(q=0;q<MAR_child;q++)
                                                                {
                                                                    QDomElement element8 = element_MAR.childNodes().at(q).toElement();
                                                                    if(element8.nodeName()=="NAM"&&element8.text()=="MI")
                                                                    {
                                                                        for(r=0;r<MAR_child;r++)
                                                                        {
                                                                            QDomElement element9 = element_MAR.childNodes().at(r).toElement();
                                                                            //扭矩
                                                                            if(element9.nodeName()=="VAL")
                                                                                MI_blc = element9.text();
                                                                            qDebug()<<"this is MI_blc"<<MI_blc;
                                                                        }
                                                                    }
                                                                    if(element8.nodeName()=="NAM"&&element8.text()=="WI")
                                                                    {
                                                                        for(r=0;r<MAR_child;r++)
                                                                        {
                                                                            QDomElement element9 = element_MAR.childNodes().at(r).toElement();
                                                                            //角度
                                                                            if(element9.nodeName()=="VAL")
                                                                                WI_blc = element9.text();
                                                                            qDebug()<<"this is WI_blc"<<WI_blc;
                                                                        }
                                                                    }
                                                                    if(element8.nodeName()=="NAM"&&element8.text()=="W+")
                                                                    {
                                                                        for(r=0;r<MAR_child;r++)
                                                                        {
                                                                            QDomElement element9 = element_MAR.childNodes().at(r).toElement();
                                                                    ...
                                                                    if(element8.nodeName()=="NAM"&&element8.text()=="W-")
                                                                    {
                                                                        for(r=0;r<MAR_child;r++)
                                                                        {
                                                                            ...
                                                                            //角度最小值
                                                                            if(element9.nodeName()=="VAL")
                                                                                WI_blc_min = element9.text();
                                                                            qDebug()<<"this is WI_blc_min"<<WI_blc_min;
                                                                        }
                                                                    }
                                                                    if(element8.nodeName()=="NAM"&&element8.text()=="M+")
                                                                    {
                                                                        for(r=0;r<MAR_child;r++)
                                                                        {
                                                                            QDomElement element9 = element_MAR.childNodes().at(r).toElement();
                                            ...
                                            if(Program != "-1")
                                            {
                                                ...
                                                if( (Cycle_CSR != CyclePrevious)||(Channel != channelPrevious) )
                                                {
                                                    ...
                                                    if(SYSS == "ING" || 1)
                                                    {
                                                        ...
                                                        qDebug()<<"VIN_PIN_SQL2"<<VIN_PIN_SQL;
                                                        ...
                                                        if(ctrl_Type != "SB356")
                                                        {
                                                            if(STATE_blc == "NOK")
                                                            {
                                                                ...
                                                                {
                                                                    GBlotErrorCount++;
                                                                    if(blotNumTemp != groupNumCh)
                                                                    {
                                                                        sendResult = false;
                                                                        blotNumTemp = groupNumCh;
                                                                        emit send99PRG();
                                                                        qDebug()<<"send99PRG"<<blotNumTemp;
                                                                    }
                                                                    else
                                                                    {
                                                                        sendResult = true;
                                                        ...
                                                        else//ControlType_1: "SB356" ControlType_2: "OFF"
                                                        {
                                                            GBoltAllNum++;//PLC线程专用
                                                            GrecEnableCount++;
                                                            if(STATE_blc == "OK")
                                                            ...
                                                            //MainWindow::fromsecondthreaddataSB356 发到主窗口处理逻辑
                                                            emit send_mainwindowSB356(MI_blc,WI_blc,STATE_blc,Channel.toInt(),num,Program);
                                                            //ConnectPLC::fromsecondthreaddataSB356 发PLC提程序号
                                                            emit sendToPLC(Program);
                                                        }
                                                        qDebug()<<"demo***"<<demo.data_model[2];
                                                        emit sendfromworkthread(DataVar);//发给数据库SqlThreadSVW2::sqlinsert存储数据
                                                        num++;
                                                    }
                                                }
                                                else
    ...
    {//心跳 解析 EVT
        ...
        {
            QDomElement element1 = node.toElement().childNodes().at(i).toElement();
            if(element1.nodeName()=="EVT")
            {
                int EVT_child = element1.childNodes().length();
                for(j=0;j<EVT_child;j++)
                {
                    QDomElement element2 = element1.childNodes().at(j).toElement();
                    if(element2.nodeName()=="STS")
                    {
                        int STS_child = element2.childNodes().length();
                        for(k=0;k<STS_child;k++)
                        {
                            QDomElement element3 = element2.childNodes().at(k).toElement();
                            if(element3.nodeName()=="ONC")
                            {
                                int ONC_child = element3.childNodes().length();
                                for(l=0;l<ONC_child;l++)
                                {
                                    QDomElement element4 = element3.childNodes().at(l).toElement();
                                    if(element4.nodeName()=="RDY")
                                    {
                                        int RDY = element4.text().toInt();
                                        //                                        qDebug() << "RDY 1" << RDY;
                                        if(RDY == 1)
                                        {//复位成功
                                            if(!RDYY)
...
//            qDebug()<<"thisissyss"<<SYSS;
            qDebug()<<"rec xml message:" << SYSS << receiveResult.size() << receiveResult;
...
//            qDebug() << "chang zhen##########################" << receiveResultDo;
            ...
            m_pTcpSocket->write(heart);
            m_pTcpSocket->waitForBytesWritten();
            qDebug()<<"rec long xml message:" << SYSS << receiveResultDo.size() << receiveResultDo;

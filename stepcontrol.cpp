...
//    }
    if( (allStep >= stepNum)&&(CS351StatusFlag) )
    {
        ...
        case 101://receive VIN条码枪扫码
            ...
            if(stepMap["scanPass"].toString() == "All" || allowPackType)
            {
    ...
    //                    emit sendReqKeybarcode(codeBuf[3],vinCode,time.toString("yyyy-MM-dd hh:mm:ss"),1);
            ...
            break;
        case 102://Tightening function 拧紧
            if(isFirstPackflag)
            {
...
//                        GProgramIndex = 0;
            ...
            break;
        case 103://wait IO input /read IO input status 等待I/O输入
            if(!waitIOInputFlag)
            {
        ...
        case 106://display PMD 显示PDM
            ...
            break;
        case 108:   //uplode tray meaages 辊道线数据绑定
        {
            ...
            break;
        case 107://air tightness test 泄漏
        {
...
//            break;
            ...
            break;
        case 149://operator pack1 操作PACK1
            ...
            break;
        case 150://operator pack2 操作PACK2
...
//            stepNum++;
            break;
        case 151://request product type 向服务器请求产品型号
...
//                        emit sendReqTightStatus("A410","A390-A","1");
            ...
            break;
        case 152://station finish 手工站站完成
            if(GPackType == "PHEV")
            {
...
{
//    qDebug()<<"IO revStopButtonSignal:"<<value<<stopValueFlag;
    if(stopValueFlag != value)
    {
        stopValueFlag = value;
        if(value)//stop signal restart 急停按钮复位
        {
            qDebug()<<"recv stop signal restart";
            emit sendUIEmergencyStop(true);
        }
        else //stop signal work 急停信号
        {
//            if(!isPopUpFlag)//pop up box 急停提醒弹窗正显示在窗口
//            {
//                isPopUpFlag = true;
                 if(vari2 != "XL"){
...
{
    qDebug()<<"informEmergencyStop:"<<isReset;
    if(isReset)
    {
//        isPopUpFlag = false;

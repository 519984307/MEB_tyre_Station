...
bool GFirstTightenFlag = true;//当前位置第一次拧紧
bool GCalibrationON = false;//标定开启/关闭

bool isPopups = false;//正在弹窗
//打印日志
...
//    QWSServer::setCursorVisible(false);//在电脑上运行要注释
    ...
    //有配置就取配置，没配置就配置一个给个默认值 开启标定功能
    if(QSettings("/config.ini", QSettings::IniFormat).contains("baseinfo/CalibrationON")){
        GCalibrationON = QSettings("/config.ini", QSettings::IniFormat).value("baseinfo/CalibrationON").toBool();
    }else{
        GCalibrationON = false;
        QSettings("/config.ini", QSettings::IniFormat).setValue("baseinfo/CalibrationON",GCalibrationON);
    }
...
//    qDebug()<<"this is variable2"<<variable2<<Gvariable2;
    ...
    Version = "V21.0917.01";    //date
    qDebug()<<"thisMEBDat@2.Version:"<<Version;
    if(GIsTestStartFlag){
        ControlType_1 = "OFF";
    }
...
//    RFID *SVWrfid = new RFID;
    qDebug()<<"start";
    GRFIDIP = configIniRead->value("baseinfo/RfidIp").toString();
    GRFIDPort = configIniRead->value("baseinfo/RfidPort").toInt();
    delete configIniRead;
//    SVWrfid->RFIDIP = RFIDIPTemp;
//    SVWrfid->RFIDPort = RFIDPortTemp;
//    SVWrfid->init();
    qDebug()<<"start1";
...
//    QObject::connect(interface,SIGNAL(sendKeyCodeInfo(int,int,QString,QString,QString,QString)),sqlthreadsvw2,SLOT(insertKeyCodeInfo(int,int,QString,QString,QString,QString)),Qt::AutoConnection);
    QObject::connect(w,SIGNAL(sendPLCVIN(QString)),PLC1,SLOT(receiveManualVIN(QString)));
    QObject::connect(PLC1,SIGNAL(sendCmdToMain(QString,int,int)),w,SLOT(VinPinCodeViewSB356(QString,int,int)),Qt::AutoConnection);
    QObject::connect(PLC1,SIGNAL(sendEnableToTighten(bool)),tighten_xml,SLOT(revEnableFromPlc(bool)),Qt::AutoConnection);
    QObject::connect(PLC1,SIGNAL(sendCurrentCode(QString)),w,SLOT(showCurrentCode(QString)));
    QObject::connect(PLC1,SIGNAL(sendPLCConnect(bool)),w,SLOT(PLCIsReady(bool)),Qt::AutoConnection);
    if(ControlType_1 != "OFF")//拧紧控制器型号
    {
        ...
        QObject::connect(w,SIGNAL(sendTighteningResult1(int,int)),PLC1,SLOT(revCmdFunc(int,int)),Qt::AutoConnection);
        QObject::connect(&inputevents,SIGNAL(sendTighteningResult1(int,int)),PLC1,SLOT(revCmdFunc(int,int)),Qt::AutoConnection);
        ...
        QObject::connect(work,SIGNAL(send_mainwindowSB356(QString,QString,QString,int,int,QString)),w,SLOT(fromsecondthreaddataSB356(QString,QString,QString,int,int,QString)),Qt::AutoConnection);

        QObject::connect(work,SIGNAL(sendToPLC(QString)),PLC1,SLOT(fromsecondthreaddataSB356(QString)));
        QObject::connect(work,SIGNAL(send99PRG()),tighten_xml,SLOT(rev99PRG()),Qt::AutoConnection);
        QObject::connect(work,SIGNAL(sendReEnable()),tighten_xml,SLOT(revReEnable()),Qt::AutoConnection);
...
//        QObject::connect(interface,SIGNAL(sendStationState(bool)),PLC1,SLOT(revStationState(bool)),Qt::AutoConnection);//receive ACK
    ...
    QObject::connect(PLC1,SIGNAL(sendMesToWindow(QString,bool)),w,SLOT(revMesFromStep(QString,bool)),Qt::AutoConnection);

//    QObject::connect(SVWrfid,SIGNAL(sendRFIDToMainWindow(QString,QString)),w,SLOT(displayScanBarcode(QString,QString)),Qt::AutoConnection);
...
//    QObject::connect(PLC1,SIGNAL(sendReqPackStatus()),w,SLOT(fisVIMHandle()),Qt::AutoConnection);//request product type
//    QObject::connect(w,SIGNAL(sendFisVinCode(QString)),PLC1,SLOT(revFisPackType(QString)),Qt::AutoConnection);//request product type
//    QObject::connect(PLC1,SIGNAL(sendChangeFisUseFlag()),w,SLOT(fisVIMChange()),Qt::AutoConnection);

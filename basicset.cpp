    ...
    ui(new Ui::BasicSet)
{
   ...
   // this->setCursor(Qt::BlankCursor); //去掉光标
    ...
    if(GCalibrationON){//标定启用
        ui->but_calibration->setStyleSheet("border-image: url(:/LCD_CS351/LCD_CS351/35_all/13.bmp);");
    }else if(GCalibrationON){//标定关闭
        ui->but_calibration->setStyleSheet("border-image: url(:/LCD_CS351/LCD_CS351/35_all/14.bmp);");
    }
...
//标定启用/关闭
void BasicSet::on_but_calibration_clicked()
{
    QString calibrationValue = ui->but_calibration->styleSheet();
    if(calibrationValue.contains("14.bmp")){//标定启用
        ui->but_calibration->setStyleSheet("border-image: url(:/LCD_CS351/LCD_CS351/35_all/13.bmp);");
        GCalibrationON = true;
        QSettings("/config.ini", QSettings::IniFormat).setValue("baseinfo/CalibrationON",GCalibrationON);
    }else if(calibrationValue.contains("13.bmp")){//标定关闭
        ui->but_calibration->setStyleSheet("border-image: url(:/LCD_CS351/LCD_CS351/35_all/14.bmp);");

...
//    enableTemp         = "<MSL_MSG><PNR>21</PNR><SDR><SER>1</SER></SDR><PDC><CIO>5</CIO><CNO>5</CNO></PDC><PID><PRT>T</PRT><PI1>VIN12345678901234</PI1></PID><PRS><PRG>1</PRG> <SIO>2</SIO> <MNO>50</MNO><NUT>0</NUT><KNR>1</KNR> </PRS><TOL><VAL>1</VAL><KNR>1</KNR> </TOL></MSL_MSG></root:ROOT>";
    enableTemp         = "<MSL_MSG><PNR>21</PNR><SDR><SER>1</SER></SDR><PDC><CIO>5</CIO><CNO>5</CNO></PDC><PID><PRT>T</PRT><PI1>VIN12345678901234</PI1></PID><PRS><PRG>1</PRG> <SIO>2</SIO> <MNO>50</MNO><NUT>0</NUT><KNR>1</KNR> </PRS><TOL><VAL>1</VAL><KNR>1</KNR> </TOL><STR><VAL>1</VAL></STR></MSL_MSG></root:ROOT>";
...
//    if(sendEnableFlag && ((GgetBoltIsOk == "Yes" && handleEnableFlag(GrecvStatusMsee)) ||(GgetBoltIsOk != "Yes")))
    if(sendEnableFlag && plcSendEnabledFlag)
    {
        if(GgetBoltIsOk != "Yes")
        {
            if(!enableFlag)
...
void TightenThread::revEnableFromPlc(bool flag)
{
    qDebug()<<"plcSendEnabledFlag=true";
    plcSendEnabledFlag = true;
    cycleFlag = flag;
}
...
void TightenThread::sendCmdToCs351(int type)
{
    ...
    QString Data_Xml_T = "";
    QString strProNo = carInfor.proNo[groupNumCh];//testPro;//;
//    QString cs351ProNo = carInfor.proNo[GProgramIndex];
    ...
    else if(type == 2)//send enable
    {
        if(ctrlType == "SB356")
        {
            xml_STR11 = "<MSL_MSG><PNR>21</PNR>";
//            xml_Grs = "<MSL_MSG><PNR>21</PNR><GRS><GRP><GNR>1</GNR><GNS>1</GNS><SIO>1</SIO> </GRP></GRS>"; //68
            xml_Grs = "<GRS><GRP><GNR>1</GNR><GNS>1</GNS><SIO>1</SIO> </GRP></GRS>";
            xml_TOL = "<TOL><VAL>1</VAL><GRS><GRP><GNR>1</GNR></GRP></GRS></TOL>";
            xml_PID = "<PID><PRT>T</PRT><PI1>*****************</PI1></PID>";
            xml_STR = "<STR><VAL>1</VAL><GRS><GNR>1</GNR></GRS></STR></MSL_MSG></root:ROOT>";
            KNR_Temp = "<KAN><KNR>1</KNR></KAN>";
            xml_PID.replace(22,VIN_PIN_SQL.size(),VIN_PIN_SQL);
            for(int i=0;i<strChannel.size();i++)
            {
                xml_Grs_KNR.append(KNR_Temp.replace(10,1,strChannel.mid(i,1)));
            }
//            qDebug()<<"############"<<xml_Grs_KNR;
            xml_Grs.replace(46,1,xml_Grs_KNR);
            if(!cycleFlag)
            {
                strProNo = "44";
            }
        ...
        else
        {
            ...
            else
            {
                ...
                qDebug()<<"VIN_PIN_SQL1"<<VIN_PIN_SQL;
//                Data_Xml_T.replace(313+35,17,"***"+VIN_PIN_SQL);
            }
            int len1 = 0;
            if(is99Flag)
...
{
    QString sendBuf = "";
    xml_Grs.replace(15,1,gNum);
    xml_TOL.replace(32,1,gNum);
    xml_STR.replace(27,1,gNum);
    sendBuf = xml_STR11 + xml_PID + xml_Grs + xml_TOL + xml_STR;
    return sendBuf;
}
...
void TightenThread::revReEnable()
{
    qDebug()<<"resendEnable";
    plcSendEnabledFlag = true;
    sendEnableFlag = true;
